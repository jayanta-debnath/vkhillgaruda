...
def update_remote_config(
    ...
    try:
        ...
        template["parameters"][param_key] = {"defaultValue": {"value": param_value}}
...
def run_command(command, retries=0, retry_delay=5):
    ...
    while attempt < max_attempts:
        ...
        if attempt < max_attempts:
            ...
            print(
                f"Command '{command}' failed after {max_attempts} attempts with error:\n{result.stderr}"
            )
            sys.exit(1)
...
def has_firebase_functions(app):
    ...
    try:
        ...
        with open(firebase_json_path, "r") as f:
            ...
            return (
                "functions" in firebase_config and len(firebase_config["functions"]) > 0
            )
    except Exception as e:
...
def release(app):
    ...
    input("Log the time taken for the release and press Enter...")
    ...
    if branch_name != "main":
        ...
        if reltype == "test":
            prefix = "TEST/"
        set_value_in_file(
            "vkhpackages/lib/common/const.dart",
            "final String dbrootGaruda",
            f' "{prefix}GARUDA_01";',
        )
        set_value_in_file(
            "vkhpackages/lib/common/const.dart",
            "final String dbrootSangeetSeva",
            f' "{prefix}SANGEETSEVA_01";',
        )
                ...
                if app == "vkhgaruda":
                    ...
                    if gdrive_path:
                        dest_path = os.path.join(
                            gdrive_path, os.path.basename(new_apk_path)
                        )
                        try:
...
def main():
    ...
    if reltype == "release" and is_temple_service_time():
        ...
        print(warning)
        print(
            "Proceeding may impact service time. Press Enter to continue or Ctrl+C to cancel."
        )
        print("!" * 70 + "\n")
        input()
